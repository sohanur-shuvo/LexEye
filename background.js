  ...
  async startRecording(tabId) {
    try {
      ...
      try {
        ...
        console.warn('[Meeting Recorder] Content script not ready:', msgError.message);

        if (msgError.message.includes('Receiving end does not exist')) {
          // Try to inject script dynamically
          console.log('[Meeting Recorder] Injecting content script...');
          try {
            ...
            console.error('[Meeting Recorder] Injection failed:', injectError);
            throw new Error('Please refresh the meeting tab and try again.');
          }
        } else {
          throw msgError;
        }
